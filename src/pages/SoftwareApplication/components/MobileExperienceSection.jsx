    ...
    <div className="flex h-full flex-col rounded-[28px] bg-gradient-to-br from-white via-[#f5f7fb] to-white p-4 text-navy shadow-inner">
      ...
      <div className="mt-5 flex flex-1 flex-col items-center justify-center text-center">
        <div className="flex h-16 w-16 items-center justify-center rounded-[22px] bg-sky-accent/20 shadow-sm">
          <img src={VHTLogo} alt="VHT logo" className="h-10 w-10 object-contain" />
        </div>
        ...
        <div className="mt-5 w-full space-y-3">
          <div className="h-11 rounded-2xl border border-border/70 bg-white px-4 text-left text-[11px] text-muted-foreground flex items-center">
            Tenant selection
          </div>
          <div className="h-11 rounded-2xl bg-navy text-white flex items-center justify-center text-[11px] font-semibold tracking-wide shadow-md shadow-navy/20">
            LOGIN
          </div>
        </div>
      </div>
    ...
    <div className="flex h-full flex-col rounded-[28px] bg-gradient-to-br from-navy via-primary-blue to-sky-accent p-4 text-white shadow-inner">
        ...
        {[
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <div className={`flex h-11 w-11 items-center justify-center rounded-full border-4 border-white/20 ${card.tone} text-[11px] font-semibold text-navy`}>
                {card.value}
              </div>
            </div>
      ...
      <div className="mt-4 rounded-3xl border border-white/20 bg-white/10 p-4 backdrop-blur-sm">
        ...
        <div className="mt-3 flex items-end gap-2">
          {[28, 54, 42, 70, 60, 86, 64].map((height, index) => (
            <motion.span
              key={index}
...
function renderPhoneScreen(id) {
  switch (id) {
    case "login":
      return <PhoneLoginScreen />;
    case "dashboard":
      return <PhoneDashboardScreen />;
    case "application":
      return <PhoneApplicationScreen />;
    case "gateway":
...
export function MobileExperienceSection({ mobileExperience }) {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => {
      setActiveSlideIndex((current) => (current + 1) % mobileExperience.slides.length);
    }, 3600);

    return () => window.clearInterval(timer);
  }, [mobileExperience.slides.length]);

  return (
    <section className="relative overflow-hidden bg-section-alt py-16 md:py-24">
      <div className="absolute left-0 top-0 h-64 w-64 rounded-full bg-white/50 blur-3xl" />
      <div className="absolute bottom-0 right-0 h-80 w-80 rounded-full bg-sky-accent/20 blur-3xl" />
        ...
        <div>
          ...
          <div className="mt-8 grid gap-3 text-base text-muted-foreground">
            {mobileExperience.highlights.map((item, index) => (
              <div key={item} className="flex items-start gap-3 rounded-2xl border border-white/70 bg-white/75 px-4 py-3 shadow-sm backdrop-blur-sm">
            ...
            <div className="overflow-hidden rounded-[2.2rem] border border-white/20 bg-white p-3">
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  className="h-[540px]"
                >
                  {renderPhoneScreen(activeSlide.id)}
                </motion.div>
              </AnimatePresence>
            </div>
